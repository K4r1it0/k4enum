...
class ActivePortScaning(BaseTask):
...
class PassivePortScaning(BaseTask):

    def requires(self):
        return ActivePortScaning(domain=self.domain,case=self.case,save_directory=self.save_directory,scan_id=self.scan_id)

    def output(self):
        return luigi.LocalTarget(f"{self.save_directory}/{self.__class__.__name__}-{self.case}")

    def run(self):
        cmd = f"cat {self.save_directory}/DNSResolving-{self.case} | naabu -silent -exclude-ports '443,80,81,300,591,593,832,981,1010,1311,1099,2082,2095,2096,2480,3000,3128,3333,4243,4567,4711,4712,4993,5000,5104,5108,5280,5281,5601,5800,6543,7000,7001,7396,7474' --passive | anew -d {self.input().path}"
        self.run_cmd(cmd, self.output().path)

class HTTPProbing(BaseTask):

    def requires(self):
        return PassivePortScaning(domain=self.domain,case=self.case,save_directory=self.save_directory,scan_id=self.scan_id)
    ...
    def run(self):
        cmd = f"cat {self.input().path} {self.save_directory}/PassivePortScaning-{self.case} | httpx -silent -t 500 -fhr -sr -store-chain -srd ~/bugbounty/archive/ | awk '{{print $1}}'"
        self.run_cmd(cmd, self.output().path)
...
class MainEnumerationTask(BaseTask):
    ...
    def run(self):
        self.update_status('done')
        self.update_scan_status('done')from base_task import BaseTask
from utils import *
import tempfile
import luigi
import requests
