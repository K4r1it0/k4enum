...
const defaultSearchValue = "Task = 'TaskA' & Task = 'TaskB' & Task = 'TaskC'";
const searchValue = ref(route.query.search || defaultSearchValue);

const submitSearch = () => {
  const searchQuery = searchValue.value.replace(/'/g, ""); // Remove single quotes
  const searchParams = searchQuery.split(/\s*&\s*/).join("|"); // Replace '&' with '|'
  page.value = 1;
  router.push({ query: { search: searchParams } });
  getTasks();
};
